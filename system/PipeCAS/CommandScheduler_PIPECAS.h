...
/********* SRT Buffer System **********/
	...
	public:
		...
		BusPacket* commandSchedule()
		{
			...
			// previous round is done, or executing outRound scheduler
			roundType = prevCAS;
			...
			if(!queueScheduled.empty()) {
				...
				// Check if the round is complete, is all the CAS commands are issued
				bool roundDone = true;
				for(unsigned int index = 0; index < queueScheduled.size(); index++)	{
			...
			if(scheduledCommand != NULL) {
				sendCommand(scheduledCommand, scheduledIndex);
